...
export function UnifiedVoiceModal({ visible, phase, expenses, onStop, onClose, onCancelAnalyzing, onSaveOne, onSaveAll, onDeleteOne, onDismissResults, saving }: Props) {
      ...
      <View style={styles.resultsContainer}>
        <View style={styles.resultsHeader}>
          ...
          <View style={styles.resultsTitleText}>
            ...
            <Text variant="small" color="textSecondary" align="center">{t('voice_reviewHint')}</Text>
          </View>
        </View>
        ...
        <ScrollView style={styles.resultsList} contentContainerStyle={styles.resultsListContent} showsVerticalScrollIndicator={true}>
          {safeDrafts.map((draft, index) => {
            ...
            return (
              <View key={`${draft.category ?? 'OTHER'}-${index}`}>
                ...
                />
                {index < safeDrafts.length - 1 && <View style={[styles.divider, { backgroundColor: theme.border }]} />}
              </View>
            );
          })}
        </ScrollView>
        ...
        {safeDrafts.length > 1 && (
          <View style={[styles.resultsFooter, { borderColor: theme.border }]}>
            <Button variant="dark" size="md" style={{ flex: 1 }} loading={saving} onPress={() => onSaveAll(safeDrafts)}>{t('voice_saveAll', { n: safeDrafts.length })}</Button>
          </View>
        )}
        <DeleteConfirm
...
const styles = StyleSheet.create({
  ...
  resultsTitleText: { width: '100%', gap: 2, alignItems: 'center' },
  resultsList: { flex: 1 },
